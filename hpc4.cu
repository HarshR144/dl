...
void printVector(int* C, int N, const string& label) {
    cout << label << " (first 10 elements): ";
    for (int i = 0; i < min(N, 10); ++i) {
        cout << C[i] << " ";
    }
    cout << "...\n";  // indicate that there's more data
}
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // Print first few elements of the result vectors
        printVector(h_C_CPU, N, "CPU Vector");
        printVector(h_C_GPU, N, "GPU Vector");
